...
class Loss(nn.Module):
    ...
    def forward(self, args, data, pred_class, mu_a, sigma_a, mu_b, sigma_b, scene_scores):
        ...
        a_nll = self.timestamp_stack(self.min_gaussian_nll, mu_a, sigma_a, y_a, valid_a)
        a_ade = self.timestamp_stack(self.min_ade, mu_a, sigma_a, y_a, valid_a)
        a_fde = self.timestamp_stack(self.min_fde, mu_a, sigma_a, y_a, valid_a)

        ## load agent_b data 
        y_b = data['y_b'].to(device)
        valid_b = data['valid_b'].to(device)
        
        b_nll = self.timestamp_stack(self.min_gaussian_nll, mu_b, sigma_b, y_b, valid_b)
        b_ade = self.timestamp_stack(self.min_ade, mu_b, sigma_b, y_b, valid_b)
        b_fde = self.timestamp_stack(self.min_fde, mu_b, sigma_b, y_b, valid_b)
        
        ## scene scores
        
        # total loss: High-level scene intention loss + keyframe distance loss + trajectory distribution loss
        loss = relation_loss + (a_fde[-1] + b_fde[-1])/2 + (a_nll[-1] + b_nll[-1])/2
