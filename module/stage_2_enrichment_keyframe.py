...
class Net(nn.Module):
    def __init__(self, config):
        ...
        # initiate decoder
        self.marg_keyframe_pred = MLP(self.hidden_dim, self.hidden_dim, self.out_dim//10)
        self.marg_pred = MLP(self.hidden_dim, self.hidden_dim, self.out_dim)
        self.cond_keyframe_pred = MLP(self.hidden_dim+self.out_dim, self.hidden_dim, self.out_dim//10)
    ...
    def forward(self, data):
        ...
        if none_score > pass_score and none_score > yeild_score:
            keyframe_a = self.marg_keyframe(x_a)
            keyframe_b = self.marg_keyframe(x_b)
            concat_a = torch.cat((keyframe_a, x_a), dim=-1)
            concat_b = torch.cat((keyframe_b, x_b), dim=-1)
            traj_a = self.marg_pred(concat_a)
            traj_b = self.marg_pred(concat_b)
        ...
        # conditional prediction
            ...
            concat_a = torch.cat((keyframe_a, x_a), dim=-1)
            traj_a = self.marg_pred(concat_a)
            concat_b = torch.cat((traj_a, x_b), dim=-1)
            keyframe_b = self.cond_keyframe(concat_b)
            concat_b = torch.cat((keyframe_b, x_b), dim=-1) 
            traj_b = self.cond_pred(concat_b)
        else:
            keyframe_b = self.marg_keyframe(x_b)
            concat_b = torch.cat((keyframe_b, x_b), dim=-1)
            traj_b = self.marg_pred(concat_b)
            concat_a = torch.cat((traj_b, x_a), dim=-1)
            keyframe_a = self.cond_keyframe(concat_a)
            concat_a = torch.cat((keyframe_a, x_a), dim=-1) 
            traj_a = self.cond_pred(concat_a)
